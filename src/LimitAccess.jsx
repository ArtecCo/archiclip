...
export default function LimitAccess({ onLimitChange }) {
  ...
  useEffect(() => {
    ...
    async function prepareInvite() {
      ...
      setOpen(true); setMode("invite"); setError(""); setMessage("");
      try {
        const snap = await getDoc(doc(db, "accessInvites", inviteToken));
        if (!snap.exists()) { if (!cancelled) setError("This invitation could not be found."); return; }
        const invite = snap.data();
        if (invite.expiresAt && Date.now() > Number(invite.expiresAt) && !invite.used) { if (!cancelled) setError("This invitation has expired."); return; }
        if (!cancelled) setEmail(String(invite.email || "").trim().toLowerCase());
      } catch (err) { console.error("Could not load invitation", err); if (!cancelled) setError("Could not load this invitation."); }
    }
    prepareInvite();
  ...
  async function loadProfile(currentUser) {
    try {
      ...
      setLimit(next); onLimitChange?.(next); await loadRequests(currentUser.email); return next;
  ...
  async function claimInvite(e){
    ...
    try {
      ...
      if(!invitedEmail)throw new Error("bad-invite");
      if(invite.expiresAt && Date.now()>Number(invite.expiresAt) && !invite.used)throw new Error("expired");
      if(email.trim().toLowerCase()!==invitedEmail)setEmail(invitedEmail);
      const grantedLimit=safeLimit(invite.maxChars,DEFAULT_LIMIT);
      let account=auth.currentUser;
      if(account){
        if(account.email?.toLowerCase()!==invitedEmail)throw new Error("wrong-account");
      } else {
        try { account=(await createUserWithEmailAndPassword(auth,invitedEmail,password)).user; }
        catch(err){ if(err?.code!=="auth/email-already-in-use")throw err; account=(await signInWithEmailAndPassword(auth,invitedEmail,password)).user; }
      }
      if(!account)throw new Error("no-account");
      const userRef=doc(db,"users",account.uid);
      await setDoc(userRef,{uid:account.uid,maxChars:grantedLimit,active:true,email:account.email,invitedAt:new Date(),inviteId:inviteToken,status:"Granted"},{merge:true});
      if(!invite.used){
        await updateDoc(doc(db,"accessInvites",inviteToken),{used:true,usedBy:account.uid,usedAt:new Date()});
      } else if(invite.usedBy && invite.usedBy!==account.uid){
        throw new Error("used-by-other");
      }
      setUser(account); setLimit(grantedLimit); onLimitChange?.(grantedLimit); setPassword("");
      setMessage(`Access activated. Your limit is ${grantedLimit.toLocaleString()} characters.`);
      window.history.replaceState({},"",window.location.pathname); setMode("account");
    } catch(err){
      console.error("Invitation activation failed",err);
      const code=err?.code;
      setError(err?.message==="expired"?"This invitation has expired.":err?.message==="wrong-account"?"Sign out and use the invited email address.":err?.message==="used-by-other"?"This invitation has already been used by another account.":err?.message==="invalid"?"This invitation could not be found.":code==="auth/invalid-credential"?"The email or password is incorrect.":code==="auth/weak-password"?"Password must be at least 6 characters.":"Could not activate the invitation. Please try again.");
    } finally { setBusy(false); }
  }
