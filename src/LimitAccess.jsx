...
export default function LimitAccess({ onLimitChange }) {
  ...
  useEffect(() => {
    ...
    async function prepareInvite() {
      ...
      try {
        ...
        if (invite.used) { if (!cancelled) setError("This invitation has already been used."); return; }
        if (invite.expiresAt && Date.now() > Number(invite.expiresAt)) { if (!cancelled) setError("This invitation has expired."); return; }
  ...
  async function claimInvite(e){
    ...
    try {
      ...
      if(invite.used||(invite.expiresAt&&Date.now()>Number(invite.expiresAt)))throw new Error("expired");
      ...
      await setDoc(doc(db,"users",account.uid),{uid:account.uid,maxChars:grantedLimit,active:true,email:account.email,invitedAt:new Date(),inviteId:inviteToken,status:"Granted"},{merge:true});
      ...
      const code=err?.code;
      setError(err?.message==="expired"?"This invitation has expired or has already been used.":err?.message==="wrong-account"?"Sign out and use the invited email address.":err?.message==="invalid"?"This invitation could not be found.":code==="auth/invalid-credential"?"The email or password is incorrect.":code==="auth/weak-password"?"Password must be at least 6 characters.":"Could not activate the invitation. Please try again.");
    } finally { setBusy(false); }
  }
